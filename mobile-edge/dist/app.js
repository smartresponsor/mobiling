...
import routeMobileCruding from "./routes/cruding/mobile.js";
import routeMobileLocale from "./routes/locale/mobile.js";
import routeMobileNavigationShell from "./routes/navigation/mobile.js";
import routeMobileMessageThread from "./routes/message/thread.js";
import routeMobileNotification from "./routes/notification/mobile.js";
import routeApplicationRuntime from "./routes/runtime/application.js";
import routeMobileSupport from "./routes/support/mobile.js";
import routeMobileVendorPayout from "./routes/vendor/payout.js";
import routeMobileVendorProfile from "./routes/vendor/profile.js";
import routeMobileVendorStatement from "./routes/vendor/statement.js";
import routeMobileVendorSummary from "./routes/vendor/summary.js";
import routeMobileVendorTransaction from "./routes/vendor/transaction.js";
import routeMobileWallet from "./routes/wallet/mobile.js";
...
app.addHook("onSend", async (request, reply, payload) => {
    const duration = Date.now() - Number(request.startedAt || Date.now());
    await M.observe(String(request.routerPath || request.routeOptions?.url || request.url), reply.statusCode || 200, duration);
    return payload;
});
...
await routeMobileAttachment(app);
await routeMobileCart(app);
await routeMobileCatalog(app);
await routeMobileCruding(app);
await routeMobileLocale(app);
await routeMobileNavigationShell(app);
await routeMobileMessageThread(app);
await routeMobileNotification(app);
await routeApplicationRuntime(app);
await routeMobileSupport(app);
await routeMobileVendorPayout(app);
await routeMobileVendorProfile(app);
await routeMobileVendorSummary(app);
await routeMobileVendorStatement(app);
await routeMobileVendorTransaction(app);
await routeMobileWallet(app);
