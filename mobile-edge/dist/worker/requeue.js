...
function jitter(v) { return Math.floor(v * (0.5 + Math.random())); }
...
function globalBudgetOk() {
    ...
    if (minuteCount >= ENV.REQUEUE_RPM)
        return false;
    if (dayCount >= ENV.REQUEUE_DAILY)
        return false;
    minuteCount++;
    dayCount++;
    return true;
}
async function perKeyBudgetOk(key) {
    if (!key)
        return true;
