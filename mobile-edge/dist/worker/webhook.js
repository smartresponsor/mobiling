...
function exp(base, cap, attempt) { let d = base * Math.pow(2, attempt); if (d > cap)
    d = cap; return jitter(d); }
async function deliver(url, secret, body) {
    const u = new URL(url);
...
export function startWebhookWorker() {
    setInterval(async () => {
        ...
        for (const d of due) {
            ...
            const sub = subs.find(x => x.id === d.subId);
            if (!sub || !sub.enabled) {
                await M.inc("whTotal", { status: "failed" });
                continue;
            }
