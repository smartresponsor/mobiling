...
import { CartingApiClient, type CartingApiErrorPayload } from "../../client/carting/cartingApiClient.js";
import { mobileAccessErrorPayload } from "../../contract/mobile/access/error.js";
import {
  mobileCartCheckoutHandoffPayload,
  mobileCartItemMutationRequest,
  mobileCartPayload,
} from "../../contract/mobile/cart.js";
...
function normalizeErrorPayload(body: unknown): CartingApiErrorPayload {
...
function cartPayload(body: unknown): Record<string, unknown> {
  if (!isRecord(body)) {
    return {};
  }
...
function normalizeCartItem(value: unknown): Record<string, unknown> {
...
function normalizeCart(body: unknown): Record<string, unknown> {
...
function normalizeCheckoutHandoff(body: unknown): Record<string, unknown> {
  const root = cartPayload(body);
