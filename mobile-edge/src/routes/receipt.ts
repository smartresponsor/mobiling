...
export default async function route(app: FastifyInstance){
  app.post("/receipt/verify", {
    ...
    const apiKey = (req.headers["x-api-key"] || req.headers["x-sr-key"] || "") as string || null;
    const store = await getStore();
    if (idem){
      const cached = await store.idemGet(idem);
      if (cached){ await M.inc("receipt", { outcome:"cached" }); return res.status(cached.status).send(cached.body); }
    }
    ...
    try {
      ...
      if (idem){ await store.idemPut(idem, out.status, out.body); }
      ...
      if (idem){ await store.idemPut(idem, 502, { error: "edge_network" }); }
